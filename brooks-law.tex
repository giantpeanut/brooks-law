  		...
  		\begin{atomize}
  			\item
				  ...
				  \item When $p_{s}(t_{d})=p_{j}(t_{d})$, either strategy is equally
						...
						curves $p_{s}$ and $p_{j}$ intersect. 

						"Write solution to $p_{s}(t_{d})=p_{j}(t_{d})$ here" 

						" Insert graph of $p_{s} vs p_{j}$ here "

						$t_{d} = \frac{-3x_{0}}{r-s} \leq 0$. This means that the two
