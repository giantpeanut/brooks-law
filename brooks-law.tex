...
\section*{Case II: extends Case I}
  ...
  \section*{Conclusion}
   ...
   following unequalities will model each case's system of equations.

   \begin{atomize}
    \item Case I: \\
    Any policy for hiring new employees will results in a earlier project completion. 
    \item Case II: \\
    Case II will pertain to brooks-law if the hiring policies follow: 
    $t_{d} < \frac{3\ell}{ks}$ \\[6pt]
    \item Case III: 
    Observing the non-dimentionalized $\psi$, we found that brooks-law is somewhat true
    in case III since there exists conditions on the non-dimentionalized $\tau$ hiring 
    case that project's progress rate exceeds that of of non-hiring case.

    An oversight is the influence that that continuous hiring has on the project 
    (even for large projects) as $\tau$ \>\> 0.
  \end{atomize}
